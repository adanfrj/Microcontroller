...
//TEST CONNECTION SERIAL ARDUINO-NODEMCU
//40040319650081
//TUGAS AKHIR
...
{ 
  ...
  //Baca permintaan dari NodeMCU untuk Arduino mengirim data
  while(Serial.available()>0)
  {
    req += char(Serial.read());
  }

  //Menghapus spasi data yang diterima
  req.trim();

  //Jika nodeMCU mengirimkan teks YA maka kirim data ke NodeMCU
  if (req == "Yes")
  {
    //kirim data
    sendDataSerial();
...
{
  unsigned long currentMillisOn2 = millis();
  unsigned long currentMillisOff2 = millis();
...
void sendDataSerial()
{
  //format data ex: 1#1#1
  String datakirim = String(jmlTotalTelurKecil) + "#" + String(jmlTotal`TelurBesar) + "#" + String (jmlTelurTotal);
